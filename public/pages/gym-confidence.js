...
export function renderGymConfidencePage() {
  const pageStyles = `
    <style>
      ...
      .gc-grid { display: grid; gap: 18px; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); align-items: stretch; }
      ...
      .gc-situations-card { min-height: 0; display: flex; flex-direction: column; max-height: 520px; }
      .gc-situations-list { display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); max-height: 420px; overflow-y: auto; padding-right: 0.4rem; margin-right: -0.4rem; }
      ...
      @media (max-width: 600px) {
        ...
        .gc-situations-card { max-height: none; }
        .gc-situations-list { max-height: none; overflow: visible; }
      }
    </style>
  `;
  const sections = `
